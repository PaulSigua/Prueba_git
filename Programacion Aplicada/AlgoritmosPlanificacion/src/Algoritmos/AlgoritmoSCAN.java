...
public class AlgoritmoSCAN {
    ...
    public AlgoritmoSCAN(List<Integer> referencias, int numMin, int numMax) {
    ...
    public void operar() {
        System.out.println("LOGARITMO SCAN");
        ...
        for (int i = 0; i < cantRef; i++) {
            ...
            if (referencia == referencias.get(referencias.size() - 1))//Si esta en la ultima
            {
                copiaReferencias.add(numMax);
                desplazamientos.add(Math.abs(referencia - numMax));
                desplazamientos.add(Math.abs(numMax - referencias.get(posReferenciaAux)-1));

                for (int j = posReferenciaAux - 1; j > -1; j--) {
